...
class UNet(nn.Module):
    def contracting_block(self, in_channels, out_channels, kernel_size=3):
        ...
        block = torch.nn.Sequential(
            BBBConv2d(kernel_size=kernel_size, in_channels=in_channels, out_channels=out_channels, padding=1),
            torch.nn.BatchNorm2d(out_channels),
            torch.nn.ReLU(),
            BBBConv2d(kernel_size=kernel_size, in_channels=out_channels, out_channels=out_channels, padding=1),
            torch.nn.BatchNorm2d(out_channels),
            torch.nn.ReLU(),
        )
        return block
    ...
    def expansive_block(self, in_channels, mid_channel, out_channels, kernel_size=3):
        ...
        """
        block = torch.nn.Sequential(
            BBBConv2d(kernel_size=kernel_size, in_channels=in_channels, out_channels=mid_channel, padding=1),
            torch.nn.BatchNorm2d(mid_channel),
            torch.nn.ReLU(),
            BBBConv2d(kernel_size=kernel_size, in_channels=mid_channel, out_channels=mid_channel, padding=1),
            torch.nn.BatchNorm2d(mid_channel),
            torch.nn.ReLU(),
    ...
    def final_block(self, in_channels, mid_channel, out_channels, kernel_size=3):
        ...
        block = torch.nn.Sequential(
                    ...
                    torch.nn.BatchNorm2d(mid_channel),
                    torch.nn.ReLU(),
                    BBBConv2d(kernel_size=kernel_size, in_channels=mid_channel, out_channels=out_channels, padding=1),
                    torch.nn.BatchNorm2d(out_channels),
                    torch.nn.ReLU()
                )
        return block
    ...
    def __init__(self, in_channel, out_channel):
        ...
        self.bottleneck = torch.nn.Sequential(
                                BBBConv2d(kernel_size=3, in_channels=mid_channel, out_channels=mid_channel * 2, padding=1),
                                torch.nn.BatchNorm2d(mid_channel * 2),
                                torch.nn.ReLU(),
                                BBBConv2d(kernel_size=3, in_channels=mid_channel*2, out_channels=mid_channel, padding=1),
                                torch.nn.BatchNorm2d(mid_channel),
                                torch.nn.ReLU(),
...
class VoxelMorph2d(nn.Module):
    ...
    def forward(self, moving_image, fixed_image):
        ...
        #print('deformation matrix size', deformation_matrix.shape)
